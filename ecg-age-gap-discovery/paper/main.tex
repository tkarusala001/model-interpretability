...
a model between them. We report it as a falsifiable pointer, not a validated
marker.
\end{abstract}
...
\item One finding that survives every check, replicated in two cohorts and
stated as a falsifiable pointer (\S\ref{sec:survives}).
\end{enumerate}
...
infarction association falls below significance while a different superclass,
ST/T change, rises above it at comparable magnitude. An analyst who had happened
to run B first would have reported a different discovery with the same
confidence, and neither association survives C. What moves between A and B is
not the biology and not the model, but which of five correlated labels the
residual happens to track in one particular partition of patients.
...
\begin{table}[t]
\centering
\caption{Segment attribution against three nulls (share of total absolute
attribution, PTB-XL). The shuffled-label control reached MAE 12.96 years against
a constant-prediction baseline of 13.21, confirming it learned nothing.}
\label{tab:nulls}
\small
\begin{tabular}{lcccc}
\toprule
...
Classical P-wave measurement --- P duration, P amplitude and PR interval ---
explains \textbf{0.7\%} of the age gap out of fold. Yet the model's per-recording
P-wave attention tracks those same measurements ($r = 0.47$ and $r = 0.43$), so it
is genuinely reading the atrial complex while extracting something those three
...
age information that this model causally depends on and that P duration, P
amplitude and PR interval do not capture. This is falsifiable, and that is its
purpose: measure P terminal force, P-wave dispersion, notching, or P-wave area,
and test whether the 0.7\% closes.
...
We have shown that three classical P-wave measurements do not capture the
dependence, not that none does, and our own $5 \to 15$ result proves the
known-feature set is still incomplete. Both cohorts are single-country, neither
has clinical outcome data, and our analysis covers adults 18--89 only. Occlusion
...
(350 tests) verifies every ground-truth claim above. Model parameters are
...
model's causal dependence on atrial information not captured by three classical
