...
the remainder. With $R^2_{\text{base}}$ from demographics alone and
$R^2_{\text{full}}$ from demographics plus known measurements,
$\text{attributable} = R^2_{\text{full}} - R^2_{\text{base}}$ and
$\text{unexplained} = 1 - R^2_{\text{full}}$. Only the unexplained part is
...
\paragraph{The error asymmetry.} Every weakness here pushes one way. Noisy
measurement, a thin known-feature set, too rigid an explainer, and an overfitted
model all \emph{shrink} the attributable share and \emph{inflate} the unexplained
residual; no mechanism produces the reverse. ``Fully explained'' is therefore
strong evidence while ``largely unexplained'' is weak evidence --- and much
published inference treats the latter as the stronger signal.
...
raises its attributable share from 14.5\% to 16.1\%, so that set is incomplete
